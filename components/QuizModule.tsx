...
export function QuizModule({ questions }: QuizModuleProps) {
        ...
        return (
            <div className="w-full max-w-2xl mx-auto">
                <Card className="text-center p-8 bg-white dark:bg-slate-900/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_0_50px_rgba(0,0,0,0.5)] border-transparent dark:border-indigo-500/20 relative overflow-hidden">
                    ...
                    <CardHeader>
                        <CardTitle className="text-4xl font-black mb-4 text-slate-900 dark:text-white uppercase tracking-tighter">
                            {passed ? t.quiz.congrats : t.quiz.tryAgain}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className={cn("text-7xl font-black mb-2 font-mono tracking-tighter drop-shadow-lg", passed ? "text-emerald-500 dark:text-emerald-400" : "text-red-500 dark:text-red-400")}>
                            {percentage}%
                        </div>
                        <p className="text-xl text-slate-600 dark:text-slate-300 mb-10 font-light">
                            {t.quiz.correct} <span className="font-bold text-slate-900 dark:text-white">{score}</span> {t.quiz.of} <span className="font-bold text-slate-900 dark:text-white">{questions.length}</span> {t.quiz.questions}
                        </p>

                        {passed && (
                            <div className="mb-8 p-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-emerald-100 dark:border-emerald-500/30 shadow-inner relative group">
                                <div className="absolute inset-0 bg-emerald-500/5 blur-xl group-hover:bg-emerald-500/10 transition-colors" />
                                <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400 mb-6 uppercase tracking-[0.2em] relative z-10 border-b border-emerald-200 dark:border-emerald-500/20 pb-2 inline-block">
                                ...
                                <p className="text-xs text-emerald-600/50 dark:text-emerald-500/50 mt-4 font-mono relative z-10 animate-pulse">
                                    {t.quiz.flipToken}
                                </p>
                            </div>
                        )}
                    ...
                    <CardFooter className="justify-center gap-4">
                        <Button onClick={handleRetry} size="lg" variant="outline" className="border-slate-200 dark:border-slate-700 bg-white dark:bg-transparent hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white">
                            <RefreshCcw className="w-4 h-4 mr-2" />
                            {t.quiz.retry}
                        </Button>
                        <Button size="lg" asChild className="bg-indigo-600 hover:bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.3)] dark:shadow-[0_0_20px_rgba(99,102,241,0.4)] border-0 px-8">
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="mb-6 flex justify-between items-center text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
